...
PROGRAM x12_BoolArrayToByteArray:
    ...
    IF vRunsCompleted > cNrRuns THEN
    	...
    	END_IF
    	//Measurement done on 1 milion packs
    	//Time to pack bool array to 4 bytes, function
    	vTest1.AverageExecTime := calcAvg(vTest1.Results);
    	//Time to pack bool array to 4 bytes, flat
    	vTest2.AverageExecTime := calcAvg(vTest2.Results);
    	//Time to pack bool array to 4 bytes, bitmask
    	vTest3.AverageExecTime := calcAvg(vTest3.Results);
    	//Time to pack bool array to 4 bytes, bitmask function
    	vTest4.AverageExecTime := calcAvg(vTest4.Results);
    	//Time to pack bool array to 4 bytes, bitmask function in a loop
    	vTest5.AverageExecTime := calcAvg(vTest5.Results);
    	//Time to pack bool array to 4 bytes, clear + bitmask
    	vTest6.AverageExecTime := calcAvg(vTest6.Results);
    	//Time to pack bool array to 4 bytes, clear + bitmask, function
